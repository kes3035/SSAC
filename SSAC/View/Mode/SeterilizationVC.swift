...
final class SeterilizationVC: UIViewController {
    //MARK: - Properties
    ...
    @objc func confirmButtonTapped(_ sender: UIButton) {
        ...
        DispatchQueue.main.async {
            let alert = UIAlertController(title: "살균 실행", message: "\(Int(self.timeStepper.value))분 동안 살균 후 자동 종료됩니다.", preferredStyle: .alert)
            let confirm = UIAlertAction(title: "확인", style: .default) { success in
                self.timeStepper.value = 30.0
                self.ventilationTimeLabel.text = "살균시킬 시간을 설정해주세요(최대 60분)"
            }
